...
# Exchange Modules Imports
from hummingbot.connector.exchange.btcex import btcex_constants as CONSTANTS, btcex_utils
from hummingbot.connector.exchange.btcex.btcex_auth import BtcexAuth
from hummingbot.connector.exchange.btcex.btcex_in_flight_order import BtcexInFlightOrder
from hummingbot.connector.exchange.btcex.btcex_order_book_tracker import BtcexOrderBookTracker
from hummingbot.connector.exchange.btcex.btcex_user_stream_tracker import BtcexUserStreamTracker

# Connector Modules Imports
...
class BtcexExchange(ExchangeBase):
    """
    BtcexExchange connects with BtcEx exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
        ...
        self._probit_auth = BtcexAuth(probit_api_key, probit_secret_key, domain=domain)
        self._order_book_tracker = BtcexOrderBookTracker(trading_pairs=trading_pairs, domain=domain)
        self._user_stream_tracker = BtcexUserStreamTracker(self._probit_auth, trading_pairs, domain=domain)
    ...
    # TODO: Make Btcex Inflight Order
    @property
    def in_flight_orders(self) -> Dict[str, BtcexInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: BtcexInFlightOrder.from_json(value)
    ...
    # TODO: A Ping route call for checking network
        ...
        while True:
            try:
                ...
                self.logger().network(f"Unexpected error while fetching trading rules. Error: {str(e)}",
                                      exc_info=True,
                                      app_warning_msg=f"Could not fetch new trading rules from {CONSTANTS.EXCHANGE_NAME}. "
                                                      "Check network connection.")
                await asyncio.sleep(0.5)

    # Not Done: Update trading rules
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = btcex_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = btcex_utils.get_new_client_order_id(False, trading_pair)
        safe_ensure_future(self._create_order(TradeType.SELL, order_id, trading_pair, amount, order_type, price))
        return order_id

    # Not Done: Check for extra work
    ...
    async def _create_order(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} {order_type.name} order to Btcex for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = BtcexInFlightOrder(
            client_order_id=order_id,
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Btcex. "
                                                      "Check API key and network connection.")
                await asyncio.sleep(0.5)

    # TODO: Write User balances
    ...
    # TODO: Write Update order status
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for order_update in order_results:
                ...
                for update in order_update["data"]:
                    order_ts: float = btcex_utils.convert_iso_to_epoch(update["time"])
                    if order_ts < min_ts:
            ...
            for trading_pair in self._trading_pairs:
                query_params = {
                    ...
                    "end_time": btcex_utils.get_iso_time_now(),
                    "limit": 1000,
                    "market_id": trading_pair
                }
    ...
    # TODO: Write Process order message
    ...
    # TODO: Write Process trade message
            ...
            for order in result["data"]:
                ...
                ret_val.append(
                    OpenOrder(
                        ...
                        time=int(btcex_utils.convert_iso_to_epoch(order["time"])),
                        exchange_order_id=order["id"]
                    )
                )
        return ret_val

    # TODO: Write Cancel all in-flight orders
